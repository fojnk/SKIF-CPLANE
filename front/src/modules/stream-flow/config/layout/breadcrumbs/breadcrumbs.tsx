...
export const MyBreadcrumbs = () => {
  const [
    ...
    SFModule.routes.project.$isOpened,
    SFModule.routes.dataSources.$isOpened,
    SFModule.routes.namespaces.$isOpened,
    SFModule.routes.namespace.$isOpened,
    SFModule.routes.editor.$isOpened,
  ]);
  ...
  const renderBreadcrumbs = () => {
    ...
    if (
      ...
      breadcrumbs.push(
        <Breadcrumbs.Item key="projects" href={SFModule.routes.root.path}>
          Projects
        </Breadcrumbs.Item>,
      );
      ...
      if (isProject || isEditor) {
        ...
        if (projectData) {
          const projectUrl = `${SFModule.routes.project.path}?id=${projectData.id}`;
    ...
    if (isDatasets) {
      breadcrumbs.push(
        <Breadcrumbs.Item
          key="datasets"
          href={SFModule.routes.dataSources.path}
        >
          Datasets
        </Breadcrumbs.Item>,
      );
    ...
    if (
      ...
      breadcrumbs.push(
        <Breadcrumbs.Item
          key="namespaces"
          href={SFModule.routes.namespaces.path}
        >
          Workspaces
        </Breadcrumbs.Item>,
      );
      ...
      if (isEditor || isNamespace) {
        ...
        if (namespaceData) {
          const href = `${SFModule.routes.namespace.path}?id=${namespaceData.id}&tab=config`;
          breadcrumbs.push(
