...
import { ShowProjectLogModel } from '@/modules/stream-flow/features/logs/show-log/project';
import { UpdateProject } from '@/modules/stream-flow/features/logs/show-log/project/ui/components';
...
export const Modal = ({
  ...
  const [data, loading, failed, pending] = useUnit([
    ShowProjectLogModel.$data,
    ShowProjectLogModel.$loading,
    ShowProjectLogModel.$failed,
    ShowProjectLogModel.$pending,
  ]);

  useEffect(() => {
    ShowProjectLogModel.load(payload.id);
    return () => ShowProjectLogModel.reset();
  }, [payload]);

  const diff = useMemo(() => {
    if (data && data.details) {
      return projectDiff(data.details);
    }
    return projectDiff();
  }, [data]);
          ...
          if (data.details.new) {
            return (
              <LogEntity
                ...
                label="Проект"
                description={data.details.new?.description || ''}
              />
            );
          }
        }
        ...
        data ? (
          <LogComment
            ...
            onSaveChanges={(comment) =>
              ShowProjectLogModel.updateComment({
                new_comment: comment ?? '',
                log_id: payload.id,
                type: 'project',
              })
