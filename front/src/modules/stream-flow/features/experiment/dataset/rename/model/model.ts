...
import { ExperimentRenameDsPayload } from '@/modules/stream-flow/features/experiment/dataset/rename';
import { streamFlowApi } from '@/modules/stream-flow/shared/api';
import { createLocalEvent } from '@/shared/lib/effector/create-local-event';
import { modalsModel } from '@/shared/ui/modals';
import { notifications } from '@/shared/ui/notifications';

type RenameRequest = streamFlowApi.dc.RequestsUpdateExperimentDatasetRequestDC;
...
const updateMutation = createMutation({
  async handler(request: RenameRequest) {
    const response =
      await streamFlowApi.experiment.v1ExperimentDatasetUpdate(request);
