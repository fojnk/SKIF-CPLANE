...
import { LogDataDC, LogsRequest } from '@/modules/control-plane/shared/types';
...
const logsQuery = createQuery({
  async handler(request: LogsRequest) {
    ...
    switch (type) {
      case 'dataset': {
        const res = await controlPlaneApi.dataset.v1DatasetLogsList({
          dataset_id: id,
          from,
          limit,
        });
      ...
      case 'namespace': {
        const res = await controlPlaneApi.namespace.v1NamespaceLogsList({
          namespace_id: id,
          from,
          limit,
        });
      ...
      case 'project': {
        const res = await controlPlaneApi.project.v1ProjectLogsList({
          project_id: id,
          from,
          limit,
        });
      ...
      case 'experiment': {
        const res = await controlPlaneApi.experiment.v1ExperimentLogsList({
          experiment_id: id,
          from,
          limit,
        });
...
sample({
  ...
  fn: ({ result }) => {
    const logs =
      (result?.logs as controlPlaneApi.dc.DtoExperimentUpdateLogDC) ?? [];
