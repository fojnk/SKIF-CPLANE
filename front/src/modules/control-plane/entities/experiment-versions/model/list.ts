...
export function create() {
  const dataQuery = createQuery({
    async handler(query: VersionsQuery) {
      const response = await controlPlaneApi.experiment.v1ExperimentVersionsList({
        ...query,
  ...
  const add = createEvent<PipeVersionDC>();
  const update = createEvent<PipeVersionDC>();
  ...
  const $data = createStore<PipeVersionDC[] | null>(null).reset(reset);
