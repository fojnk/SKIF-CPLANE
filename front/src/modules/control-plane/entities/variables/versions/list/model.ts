...
import { VariableVersion } from '@/modules/control-plane/shared/types';

type listQuery = controlPlaneApi.dc.V2ExperimentVariableVersionsListParamsDC;

const listQuery = createQuery({
  async handler(query: listQuery) {
    const response =
      await controlPlaneApi.experiment.v2ExperimentVariableVersionsList(query);
