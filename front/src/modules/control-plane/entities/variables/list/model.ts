...
type VariableType = controlPlaneApi.dc.DtoExperimentVariableShortDC;

type ResponseType = controlPlaneApi.dc.ResponsesGetExperimentVariablesResponseDC;

const variablesQuery = createQuery({
  async handler(experiment_id: number) {
    const response = await controlPlaneApi.experiment.v1ExperimentVariablesList({
      experiment_id,
    });
