...
const createExperimentVariableMutation = createMutation({
  async handler(
    request: controlPlaneApi.dc.RequestsCreateExperimentVariableRequestDC,
  ) {
    const response =
      await controlPlaneApi.experiment.v1ExperimentVariableCreate(request);
...
const createExperimentVariable =
  createEvent<controlPlaneApi.dc.RequestsCreateExperimentVariableRequestDC>();
