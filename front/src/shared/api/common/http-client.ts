...
import { createRandomString } from '@/shared/lib/common/utils';
...
export const isHttpBadResponse = (
  response: unknown,
): response is HttpBadResponse => {
  return typeof response === 'object' && !(response as Record<string, any>)?.ok;
};
...
export class HttpClient<SecurityDataType = unknown> {
  ...
  protected mergeRequestParams(
    ...
    return {
      ...
      ...(params2 || {}),
      headers: {
        'X-Request-Id': createRandomString(),
  ...
  private createResponseFormatter =
    <T, E>({
      responseFormat,
      cancelToken,
    }: {
    ...
    async (response: Response): Promise<HttpResponse<T, E>> => {
  ...
  public request = async <T = any, E = any>({
    ...
    const responseFormatter = this.createResponseFormatter<T, E>({
      responseFormat,
      cancelToken,
    });
