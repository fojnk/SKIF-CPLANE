...
**Project** принадлежит **namespace** и объединяет эксперименты, датасеты (опционально `project_id`) и JSON-конфиг с версионированием. Поддерживаются закреплённые проекты пользователя (**pinned**). Переменные пайплайна задаются только на уровне эксперимента (`t_experiment_variable`).
