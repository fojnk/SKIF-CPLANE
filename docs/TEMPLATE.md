...
- термины используются последовательно (например, `orch_id`, `apply`, `template_v_id`);
- в разделе **HTTP API** перечислены только актуальные и поддерживаемые ручки;
- для каждого важного endpoint есть ссылка на handler или пакет;
- в разделе **Сервис** описан эффект методов, а не только их названия;
- в конце есть блок **См. также** с 2-4 релевантными ссылками.
