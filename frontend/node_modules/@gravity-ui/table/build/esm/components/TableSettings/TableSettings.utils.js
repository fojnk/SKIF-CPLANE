...
const filterColumns = (column) => Boolean(column);
const orderItem = (item, state) => {
...
export const findContainer = (itemsMap, id) => {
    if (!id)
        return undefined;
    return Object.keys(itemsMap).find((key) => itemsMap[key].includes(id.toString()));
};
export const useOrderedItems = (items, orderState, setOrderState) => {
    const [activeDepth, setActiveDepth] = React.useState();
...
export const orderStateToColumnOrder = (state) => {
    const result = [];
...
export const getInitialOrderItems = (treeItems, initialOrder) => {
    const orderMap = Object.fromEntries(initialOrder.map((id, index) => [id, index]));
    const stack = [...treeItems];
    const result = { root: treeItems.map(({ id }) => id) };
    const parentNodes = [];
    while (stack.length) {
        const item = stack.shift();
        if (!item)
...
export const isDisplayedColumn = (column) => !column.columnDef.meta?.hideInSettings;
export const isFilteredColumn = (title, search) => (typeof title === 'string' ? title.toLowerCase().includes(search.toLowerCase()) : false);
export const getNestedColumnsCount = (column) => {
...
//# sourceMappingURL=TableSettings.utils.js.map
