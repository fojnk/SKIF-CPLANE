...
const index_js_1 = require("@gravity-ui/date-utils/build/dayjs/index.js");
...
function getSectionPlaceholder(sectionConfig, currentTokenValue, t) {
    switch (sectionConfig.type) {
        case 'year': {
            return t('year_placeholder').repeat((0, date_utils_1.dateTime)().format(currentTokenValue).length);
        }
        case 'quarter': {
...
function splitFormatIntoSections(format, t = i18n_1.i18n, lang = 'en') {
    const sections = [];
    const localeFormats = index_js_1.dayjs.Ls[lang].formats;
    const expandedFormat = (0, date_utils_1.expandFormat)(format, localeFormats);
...
function useFormatSections(format) {
    ...
    const [sections, setSections] = react_1.default.useState(() => splitFormatIntoSections(format, t, lang));
    const [previous, setFormat] = react_1.default.useState({ format, lang });
