...
function getShortLocalizedFormatFromLongLocalizedFormat(format) {
    return format.replace(/(\[[^\]]+])|(MMMM|MM|DD|dddd)/g, function (_, escapeSequence, localizedFormat) {
        return escapeSequence || localizedFormat.slice(1);
    });
}
var localizedFormattingTokens = /(\[[^\]]*])|(LTS?|l{1,4}|L{1,4})/g;
function expandFormat(format, formats) {
    ...
    var result = format;
    for (var i = 0; i < 5; i++) {
        var expandedFormat = result.replace(localizedFormattingTokens, function (_, escapeSequence, localizedFormat) {
            if (localizedFormat) {
