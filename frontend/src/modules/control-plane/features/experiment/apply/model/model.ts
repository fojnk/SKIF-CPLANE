...
const applyMutation = createMutation({
  async handler(experiment_id: number) {
    const response =
      await controlPlaneApi.experiment.v1ExperimentConfigApplyUpdate({
        experiment_id,
      });
...
sample({
  ...
  filter: (response) =>
    response.url.includes('control-plane') && response.url.includes('/api/v1/experiment/config/apply'),
