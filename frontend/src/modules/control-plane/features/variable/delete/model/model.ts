...
import { VariableDeletePayload } from '@/modules/control-plane/features/variable/delete/types';
...
const deleteMutation = createMutation({
  async handler(variable_id: number) {
    const response = await controlPlaneApi.experiment.v1ExperimentVariableDelete({
      variable_id,
    });
