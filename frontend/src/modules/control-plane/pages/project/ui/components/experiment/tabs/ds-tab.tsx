...
import { ExperimentDsLinkDC } from '@/modules/control-plane/shared/types';
  ...
  const canAddDataset = useMemo(
    () =>
      (experiment?.rights ?? []).includes(
        controlPlaneApi.dc.AclRightDC.RightCreateDataset,
      ),
    [experiment?.rights],
  );
...
export const DsTab = ({ experiment_id, project_id }: Props) => {
  ...
  const canEditName = rights.includes(
    controlPlaneApi.dc.AclRightDC.RightDeleteDataset,
  );
      ...
      template: (item: ExperimentDsLinkDC) =>
        item.project_id && item.project_name ? (
          <Link
            ...
            to={`${ControlPlaneModule.routes.project.path}?id=${item.project_id}`}
            onClick={(e) => {
    ...
    },
    {
      ...
      template: (item: ExperimentDsLinkDC) =>
        item.project_id && item.dataset_id ? (
          <Link
            className="g-link g-link_view_normal"
            to={`${ControlPlaneModule.routes.project.path}?id=${item.project_id}&selected=ds-${item.dataset_id}`}
            onClick={(e) => {
              if (project_id === item.project_id) {
                e.preventDefault();
